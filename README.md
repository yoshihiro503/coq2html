...
`-file-graph` _FILE_ | Show the dependency graph of <dot-file> on the index.html
...
The dot-file of file-graph could be generated by `rocq dep` and `ocamldot` commands.
...
find . -not -path '*/.*' -name "*.v" -or -name "*.glob" | xargs rocqnavi \
	...
	-file-graph html/dependency_graph.dot \
