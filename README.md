...
* generation of indexes (like `rocq doc`)
* clickable notations (like `rocq doc`)
* an option `-Q <dir> <coqdir>` to map physical dir to logical coqdir
* Markdown and LaTeX notations in comments
* darkmode
* a sidebar that displays the tree of modules

[Examples of documentation generated using Rocqnavi](https://yoshihiro503.github.io/rocqnavi/)

**Warning:** To produce cross-references, rocqnavi reads `.glob` files produced by Rocq. The format of `.glob` files is [documented](https://github.com/coq/coq/blob/master/interp/dumpglob.mli). `.glob` files exists since Coq 7.3 (2002-05) but their format has been changing silently between major releases of Coq; it was first documented officially in 2021.
...
`-show-type-information-using-rocq-lsp`       | Show type information of definitions as a tooltip using rocq-lsp
`-doc-source-url`          | The Link to the source repository

The dot-file of structure-graph could be generated by `HB.graph` vernacular command.
For dependency graphs, you can either:
- Use `-file-graph-from-depend` with a `.depend.d` file (generated by the build system with `rocq dep`), which automatically generates the graph, or
- Use `-file-graph` with a pre-generated `.dot` file.

Users of the option `-no-css` are expected to provide their own style sheet.  It must be named rocqnavi.css and it must reside in the directory where rocqnavi generates its files.

### Usage example

Let us assume that the project [MathComp-Analysis](https://github.com/math-comp/analysis) is in the directory `analysis`.
...
Here, `depend.d` is generated by the build system (e.g., using `rocq dep` with appropriate options).

### Additional details about HTML generation

<details><summary>(click to expand)</summary><p>

In addition to HTML files, rocqnavi also produces two auxiliary files:
...
For a source file `F.v` in the current directory, the generated HTML file is named `F.html`.  If the source file is in a subdirectory, a Rocq-Style fully-qualified name is used as the name of the generated HTML file, as shown below:

`-Q`               |  Source file name     |   Generated HTML file name
-------------------|-----------------------|---------------------------
none               | `F.v`                 | `F.html`
none               | `D/E/F.v`             | `D.E.F.html`
`-Q A.B.C src`     | `src/F.v`             | `A.B.C.F.html`
`-Q A.B.C src`     | `src/D/E/F.v`         | `A.B.C.D.E.F.html`

This file naming convention is compatible with that of `rocq doc` and ensures that file names are globally unique across multiple Rocq libraries and packages.
...
Earlier versions of rocqnavi used short names `F.html` instead of fully-qualified names to name the generated HTML files.  For backward compatibility, the `-redirect` option can be given.  It causes an additional `F.html` file to be generated, containing a HTTP redirection to the file `A.B.C.D.E.F.html`.

</p></details>

### Additonal details about cross-referencing
...
A cross-reference file `F.glob` is generated by the Rocq compiler coqc when it processes the `F.v` source file. When giving `F.v` as argument to rocqnavi, also give `F.glob` as argument, so that rocqnavi can use those cross-references to produce hyperlinks.

**Important:** if the source files are compiled within a Rocq namespace, you must give a `-Q` option to rocqnavi indicating this namespace. For example, if you compile with
```console
        rocq compile -Q A.B.C .
```
...
The cross-references to the Rocq standard library use the online version of this library at https://rocq-prover.org/library/, which corresponds to the latest release of Rocq.  If you wish to reference a specific version of the standard library, use the `-coqlib` option, e.g.
```console
        rocqnavi -coqlib https://rocq-prover.org/doc/V9.0.0/stdlib
```
for the 9.0.0 version of the standard library.

Using the `-external` option, you can add cross-references to other external libraries whose rocq doc or rocqnavi-generated documentation is accessible online.  For example,
```console
        rocqnavi -external https://math-comp.github.io/htmldoc_2_3_0 mathcomp ...
```
should produce cross-references to the `mathcomp` library modules.

</p></details>

### Additional details about the display of proof scripts
...
Documentation comments of the `(**md` kind are interpreted as Markdown input with possibly LaTeX mathematical notations (between `$` and `$`) inserted.

In addition, comments formatted as follows (80 characters wide)
```
     (**md***************************************************************************)
     (*                                                                             *)
     (* Some Markdown + LaTeX comment.                                               *)
     (*                                                                             *)
     (*******************************************************************************)
```
are displayed inside a frame.

### FAQ: Markdown non-standard features useful to format Rocq documentation

This section documents a few Markdown extensions and conventions that are handy when writing Rocq documentation.

```rocq
(**md
This is regular *Markdown* text: it supports **bold**, `code`, lists,
and links, in addition to the extensions described below.
*)
```
...
Inline formulas are delimited by `$...$` and display (block) formulas by `$$...$$`; both are rendered using [KaTeX](https://katex.org/).

#### deflist

The [markdown-it-deflist](https://github.com/markdown-it/markdown-it-deflist) extension is enabled, so definition lists can be written as follows:

```rocq
(**md
`enum_prob X k`
:   probability of the `k`-th value in the range of `X`
*)
```
...
See the [MathComp-Analysis wiki](https://github.com/math-comp/analysis/wiki/How-to-document) for more conventions and examples used in practice.



### Markup language for coq2html (Deprecated: use markdown instead)
...
Many of them are subsumed by the Markdown language available in Markdown + LaTeX documentation comments which
