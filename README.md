...
`-hierarchy-graph` _FILE_  | Show the hierarchy graph of <dot-file> on the index.html (You have need Graphviz command line tool)
`-dependency-graph` _FILE_ | Show the dependency graph of <dot-file> on the index.html
...
The dot-file of hierarchy-graph could be generated by `HB.graph` vernacular command.
The dot-file of dependency-graph could be generated by `rocq dep` and `ocamldot` commands.
...
find . -not -path '*/.*' -name "*.v" -or -name "*.glob" | xargs rocqnavi \
	...
	-dependency-graph html/dependency_graph.dot \
	-hierarchy-graph html/hierarchy_graph.dot \
...
Here, dependency_graph.dot was produced by a custom perl program.
